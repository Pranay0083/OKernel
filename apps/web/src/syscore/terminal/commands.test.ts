...
describe('SysCore Terminal Executor', () => {
  ...
  it('should handle syscore.mem (Stub)', () => {
    ...
    const content = result!.content as { props: { children: string } };
    expect(content.props.children).toContain('Module not loaded');
  });

  it('should resolve algorithm info via syscore API', () => {
    const result = execute('syscore.algos.rr.info');
    expect(result!.type).toBe('response');
    const content = result!.content as { component: string; data: { description: string } };
    expect(content.component).toBe('AlgoInfo');
    // The data object is algo.info, which contains description
    expect(content.data.description).toContain('Round Robin');
  });

  it('should handle case-insensitive API calls', () => {
    const result = execute('SysCore.Algos.RR.Info');
    expect(result!.type).toBe('response');
    const content = result!.content as { component: string };
    expect(content.component).toBe('AlgoInfo');
  });
